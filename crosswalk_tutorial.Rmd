...
use_crosswalk <- tribble(
  ~year, ~election, ~crosswalk_year,
	...
	2013, "primary", 2013,
	2013, "general", 2013,
	2014, "primary", 2016, # Broken
	2014, "general", 2016, ## maybe?
	2015, "primary", 2016,
	2015, "general", 2016,
	2016, "primary", 2016,
	2016, "general", 2016,
...
for(i in 1:nrow(use_crosswalk)){
  ...
  missing_g1 <- sum(!(unique(election_df$warddiv) %in% unique(crosswalk[,idcol])))
  missing_g2 <- sum(!(unique(crosswalk[,idcol]) %in% unique(election_df$warddiv)))
  if(missing_g1 > 0 || missing_g2 > 0){
    ...
    elections_with_mistakes <- bind_rows(
      ...
      data.frame(
        ...
        g1_missing_from_crosswalk=missing_g1,
        crosswalk_missing_from_g1=missing_g2
      )
    )
  }
