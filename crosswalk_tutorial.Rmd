...
We have two types of transformations to make. Some divisions from 2016 get split up into multiple 2019 divisions: 2016's division 21 becomes 21, 33, and 34. Also, parts of some 2016 divisions get combined to form new ones: the new division 10 consists of paste of 2016 10 and 26.

## Areal Weighting

Crosswalking values is usually done pretty simply: we take the counts from the 2016 data, and apportion them to 2019 boundaries. Usually, we do this by calculating the area of the intersection ("areal weighting"):
```{r prop_area}
...
The above shows us that Division 05-21 in 2016 was split up into five divisions for 2019. Two of them--1802 and 0530--look like noise; the areas are tiny and probably represent a trivial move in the boundary. 
...
I usually do one better than areal weighting: population weighting. Instead of calculating the area of the intersection, I calculate the population living within each intersection. We then apportion 2016 counts proportionally to the population in each boundary. For example, if we knew that all of 2016's division 21 lived in the Southwest of the division, then we would apportion all of its votes to new division 34.
...
for(i in 1:nrow(use_crosswalk)){
  ...
  if(crosswalk_year == "2019"){
    ...
    crosswalk <- read.csv(
      ...
      colClasses = c(
        rep("character", ifelse(crosswalk_year == "2019", 1, 2)), 
        rep("numeric", 3))
    )  
...
Similarly, the 2018 primary had two divisions that weren't in the 2016 map, 0530 and 0531, but was missing four divisions that were used in the general: 1818, 0532-0534. I use the 2019 boundaries, to avoid losing votes. Instead, 1818 et al will be assumed to have 0 votes (which isn't great either).

The fact that only 2009 is flagged isn't dispositive proof that the other crosswalks were perfect: the boundaries could have moved without creating or removing division numbers, which is all that we're checking. 
